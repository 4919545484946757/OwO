...
- 桌面操作迭代（P3/computer-use）：动作图新增 `launch`（主动打开应用/URL，不依赖前台）；修复 `inject` 在 Windows 上 handle=0 失效（改为直接注入当前焦点）；新增 `POST /perception/tree` 深度 UI 树转储（默认 depth 12 / 1000 节点）；`SemanticAnchor` 新增 `parent` 父容器约束（消除同名节点歧义）；验证谓词新增 `ui:<文本>`（UIA 树断言，等价于截图核对）与 `value:<文本>`（输入回读）；`qq-send-file` 技能包按 NTQQ 实测锚点重写（搜索框→联系人→文件→对话框→发送）。
