...
| QQ 真实桌面实测（本机） | ⚠️ 发送未实锤（用户确认消息留在输入框） | 教训：步骤 ok ≠ 发送成功；QQ 编辑器不支持 ValuePattern、同名节点多、搜索有双 UI（内联/综合搜索窗口）、会话行可见性受滚动影响；OCR 验证通道已打通（屏幕 1636 字符可读+坐标），可用于最终“截图验证”；文件/表情/红包/微信仍待受控环境复验 |
