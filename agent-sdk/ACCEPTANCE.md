...
| 桌面操作迭代（launch/Inject/tree/parent/ui-verify） | ✅ | `ActionType::Launch`；`inject` handle=0 修复；`POST /perception/tree`；`SemanticAnchor.parent` 父容器约束；`ui:`/`value:` 验证谓词；`qq-send-file` 图重写（含 parent）；契约测试 102 全绿 |
| QQ 真实桌面实测（本机，UIA 树级验证） | ✅ 文本已实锤 / ⚠️ 文件待复验 | **文本消息实锤**：输入→`ui:` 树断言→发送→失焦后会话预览变为“张子豪OwO 验证消息-已发送确认4”（消息确实上屏）；根因修复：QQ 编辑器不支持 ValuePattern（改用 ui 树断言）、同名节点歧义（parent 约束）、搜索双 UI（主窗口内联 vs 综合搜索窗口）；文件发送：曾成功打开系统对话框→输入路径→Enter→点发送（步骤 ok），但当时目标会话存疑，可靠路径下尚未完成最终验证；表情面板/红包面板为 QQ 自绘 WebView，UIA 不可达（迭代项：OCR+坐标点击）；微信未安装待测 |
