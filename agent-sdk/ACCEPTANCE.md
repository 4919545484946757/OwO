...
| QQ 真实桌面实测（本机） | ✅ 文本发送实锤（受控验证） | 受控路径：点输入框（坐标）→ 注入 → Enter；**UIA 树在消息列表区域（y=649，输入框 y≈800+）出现消息文本“OwO 受控发送验证-081”**，判定为聊天记录而非输入框残留；教训：步骤 ok ≠ 发送成功，之前残留输入框是因为点到了别的会话/别的元素；QQ 编辑器不支持 ValuePattern、同名节点多、搜索双 UI、会话行受滚动影响；OCR 通道已打通（1636 字符+坐标框）；文件/表情/红包/微信仍待复验 |
